...
// can be imported in environments (e.g. Vercel build step) where the key is
 ...
 * Shared by the local dev/prod server (server.ts) and the Vercel
...
Keep the tone encouraging, highly academic, yet simple and actionable. Use rich formatting and highlight feedback items using bold lettering.`;
  ...
  app.post("/api/ai/chat", async (req: Request, res: Response) => {
    try {
      ...
      const prompt = `You are the Brainy Library AI Assistant, a warm and highly knowledgeable academic librarian for the 'Brainy' student portal.
Help the student discover, understand, and study from the library catalog. Recommend specific items from the catalog when relevant, explain concepts, and give study guidance.
...
Reply in a friendly, encouraging, and academic tone. Use rich markdown formatting and bold the titles of any catalog items you reference.`;
